...
setup(name='pollux',
      ...
      zip_safe=False,
      data_files = [('/etc/pollux/',    'conf/config.json'        ),
                    ('/etc/pollux/',    'conf/sensors.json'       ),
                    ('/etc/pollux/',    'conf/sensors_list.json'  ),
                    ('/etc/sudoers.d/', 'conf/pollux.sudoers'     ),
                    ('/etc/defaults/',  'conf/pollux_webserv.conf')],
      install_requires=[
          # -*- Extra requirements: -*-
          'mako',
          'bottle'
